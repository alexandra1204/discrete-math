...
WORD = "Pentium"
BLOCK_BITS = 32
"""вывести блоки по отдельности до и после"""
# Кодируем сообщение
encoded = encode(WORD, BLOCK_BITS)
print(f"Исходное закодированное сообщение:\n{encoded}")

# Имитируем ошибки (5-й бит первого блока и 21-й бит второго блока)
control_bits_count = int(math.log(BLOCK_BITS, 2)) + 1
block_size = BLOCK_BITS + control_bits_count

error_positions = [4, block_size + 20]  # Индексы сломанных битов (начиная с 0)
